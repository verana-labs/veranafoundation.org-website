...
Wise Business can't create payment links by API, but incoming credits **can be read back**. The statement endpoint is closed to us (Wise retired signature-SCA for personal API tokens under PSD2), so the reconciler uses the path Wise support recommends, verified live for both Wise-to-Wise and external SEPA pushes: the **activities feed** indexes every settled credit with a **TRANSFER resource id**, and `GET /v1/transfers/{id}` (not SCA-gated) returns the **payment reference** + settled amount. `wise-reconcile.ts` regex-scans the reference for our invoice number (`{prefix}YYYY-NNNN` — banks append scheme junk like `/SCTINST…`, so never compare whole strings), verifies currency + amount, and calls the same `markInvoicePaid` as the admin path (membership activation + receipt email included). Triggers: the Wise **`balances#credit` profile webhook** (`/api/webhooks/wise`, RSA-signature-verified) and a **daily cron backstop** (`/api/cron/wise-reconcile`, `CRON_SECRET`). The scan window re-covers ~14 days and settlement is idempotent (keyed on the Wise **transfer id** as `Payment.providerRef`). Anything unsafe to auto-settle — unknown reference, underpayment, duplicate payment, or a **reference-less credit whose amount exactly matches an open invoice** — emails the admin allowlist for manual review; admin mark-paid stays the fallback. Setup: just a read-only API token + profile id (no SCA keypair).
